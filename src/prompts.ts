...
/// GPTに送るプロンプト集。input.render()でプレースホルダに変数が差し込まれる。
/// Linked IssueはFK追加文。GPTはLinked Issueが何なのかを学習済みなので、Linked Issueに関する余計なInstructionは不要(ChatGPT-4o談)
...
- Evaluate if the changes address the issues or requirements described in the Linked Issue.
  If the changes do not fully address these, triage it as \`NEEDS_REVIEW\`.
- If a Linked Issue is present, evaluate if the changes address the issues or requirements described in the issue.
  If the changes do not fully address these, triage it as \`NEEDS_REVIEW\`.
...
`
  ...
  // イベント pull_request_review_comment の created で使のみわれるようだ
